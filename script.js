...
function clearLegacyTeamCredentials() {
  localStorage.removeItem(TEAM_CREDENTIALS_KEY);
}
...
function getCurrentUserRecord() {
  return findLocalTeamUser(sessionStorage.getItem(`${SESSION_KEY}-email`)) || findLocalTeamUser(getCurrentUserName());
}
...
function isCashierUser() {
  const role = normalizeLoginName(getCurrentUserRole()).replace(/\s+/g, "");
...
function setAuthenticatedUser(authUser, profile = null) {
  ...
  sessionStorage.setItem(`${SESSION_KEY}-user`, getLoginDisplayName(displayName));
  sessionStorage.setItem(`${SESSION_KEY}-role`, profile?.cargo || authUser?.app_metadata?.cargo || "");
  if (authUser?.email) sessionStorage.setItem(`${SESSION_KEY}-email`, authUser.email);
}
...
async function loadUserProfile(authUser) {
  ...
  try {
    let query = supabaseClient.from(USERS_TABLE).select("id, nome, email, cargo, foto_perfil, created_by, created_at");
...
async function verifyCurrentPassword(password) {
  ...
  const email = authUser?.email || sessionStorage.getItem(`${SESSION_KEY}-email`);
  if (!client?.auth || !email || !password) return false;
...
async function setupLogin() {
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", async (event) => {
      ...
      const email = form.get("email") || form.get("nome");
      const loginOk = await validateLogin(email, form.get("senha"));
...
function renderCurrentUser() {
  ...
  if (avatar) {
    if (user?.foto_perfil && isHttpUrl(user.foto_perfil)) {
      ...
      createPrivateStorageUrl(window.HUB_SUPABASE?.chatFilesBucket || "hub-chat-files", user.foto_perfil)
        .then((signedUrl) => {
...
function renderItemSizeOptions(type = "epi", selectedSize = "") {
  const sizeValues = type === "uniforme"
    ? ["PP", "P", "M", "G", "GG", "EG"]
    : ["Nao se aplica", "PP", "P", "M", "G", "GG", "EG"];
...
function createChamadoEpiRow(nome = "", quantidade = "", tamanho = "Nao se aplica") {
  ...
  return `
    <div class="epi-row">
      ...
      <label>Tamanho
        <select name="epi_tamanho[]" required>${renderItemSizeOptions(tipo, tamanho)}</select>
      </label>
      <button class="danger-button remove-epi" type="button" aria-label="Remover EPI">Remover</button>
    </div>
  `;
...
function isPublicInsertOnlyCollection(collection) {
  return isPublicPage() && !isAuthenticated() && ["denuncias", "chamados", "candidaturas"].includes(collection);
}
...
async function addItem(collection, values) {
  ...
  try {
    ...
    if (isPublicInsertOnlyCollection(collection)) {
      ...
      if (error) {
        ...
        } else if (collection === "candidaturas" && isMissingColumn(error, "telefone")) {
          const { error: retryError } = await supabaseClient
            .from(TABLES[collection])
...
async function saveTeamUser(values) {
  ...
  try {
    const { data: existingRows, error: findError } = await supabaseClient
      ...
      .or(`email.ilike.${email},nome.ilike.${nome}`)
      .limit(1);
    ...
    let query = existing
      ? supabaseClient.from(USERS_TABLE).update({ nome, email, cargo, created_by: getCurrentUserName() }).eq("id", existing.id)
      : supabaseClient.from(USERS_TABLE).insert({ nome, email, cargo, created_by: getCurrentUserName() });

    let result = await query.select("*");

    if (result.error && isMissingCreatedByColumn(result.error)) {
      query = existing
        ? supabaseClient.from(USERS_TABLE).update({ nome, email, cargo }).eq("id", existing.id)
        : supabaseClient.from(USERS_TABLE).insert({ nome, email, cargo });
    ...
    upsertLocalUser({ ...saved, email: saved.email || email, cargo: saved.cargo || cargo, syncStatus: "active" });
    setSyncStatus("Supabase EIXO online", true);
...
async function updateCurrentAccount(currentPassword, newName, newPassword, newFotoUrl) {
  ...
  try {
    let payload = {
      ...
      email: updatedUser.email || sessionStorage.getItem(`${SESSION_KEY}-email`) || null,
    ...
    if (result.error && (isMissingColumn(result.error, "cargo") || isMissingColumn(result.error, "foto_perfil") || isMissingColumn(result.error, "email"))) {
      const legacyPayload = { nome: payload.nome, created_by: payload.created_by };
      ...
      result = await query.select("*");
      showModal("Atualização parcial", "Os dados principais foram alterados. Atualize o banco para salvar email, cargo e foto de perfil.", "info");
    } else if (result.error) {
      throw result.error;
    }

    const saved = mapRows("usuarios", result.data || [])[0] || updatedUser;
    upsertLocalUser({ ...updatedUser, ...saved });
    if (newName) sessionStorage.setItem(`${SESSION_KEY}-user`, getLoginDisplayName(updatedUser.nome));
    setSyncStatus("Supabase EIXO online", true);
    return true;
  } catch (error) {
...
function renderDashboard() {
  ...
  const dashboardItems = [
    ...
    ...data.malotes.map((item) => ({ type: "malote", source: item, date: item.createdAt })),
  ...
  if (dashboardTarget) {
    if (dashboardItems.length === 0) {
      ...
      dashboardTarget.innerHTML = dashboardItems
        .map((item) => item.type === "malote"
          ? `<li>${renderMaloteCardContent(item.source, { showAudit: false, showActions: false })}</li>`
          : `<li><div class="item-topline"><p class="item-title">${escapeHtml(item.title)}</p><span class="${badgeClass(item.tag)}">${escapeHtml(item.tag)}</span></div><p>${escapeHtml(item.text)}</p><p class="item-meta" style="margin-top: 6px; font-size: 12px;">${escapeHtml(item.date)}</p></li>`)
        .join("");
    }
  }
...
function renderTeamUsers() {
  ...
  renderCards("usuarios-list", users, (item) => `
    <article class="item-card">
      ...
      <p class="item-meta" style="margin-top: 8px;">E-mail: ${escapeHtml(item.email || "Cadastre no Supabase Auth")}</p>
    ...
    if (candidaturas.length > 0) {
      candidaturasHtml = candidaturas.map(c => `
        <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid var(--line); display: flex; justify-content: space-between; align-items: center;">
          ...
          <button type="button" class="secondary-link" data-private-storage-bucket="hub-curriculos" data-private-storage-path="${escapeHtml(c.curriculo_url)}" style="min-height: 28px; padding: 0 10px; font-size: 12px;">Ver Currículo</button>
        </div>
      `).join("");
    }
...
function getAuthorAvatar(authorName) {
  ...
  if (user && user.foto_perfil && isHttpUrl(user.foto_perfil)) {
  ...
  target.innerHTML = messages
    .map((item) => {
      const attachment = item.arquivo
        ? `<button class="attachment-chip" type="button" data-private-storage-bucket="${escapeHtml(window.HUB_SUPABASE?.chatFilesBucket || "hub-chat-files")}" data-private-storage-path="${escapeHtml(item.arquivo.url || "")}">Arquivo: ${escapeHtml(item.arquivo.name)} ${escapeHtml(formatFileSize(item.arquivo.size))}</button>`
        : "";
...
document.addEventListener("click", (event) => {
  ...
  if (privateFileButton) {
    event.preventDefault();
    openPrivateStorageFile(privateFileButton.dataset.privateStorageBucket, privateFileButton.dataset.privateStoragePath);
    return;
  }

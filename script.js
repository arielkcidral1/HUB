...
/* =========================================================
   MELHORIA — VAGAS E PROJETOS
   Filtros por unidade, nome do candidato e CPF
...
(function () {
  ...
  let client = null;
  let vagasAdmin = [];
  let candidaturasAdmin = [];
  let renderizandoVagasAdmin = false;
  ...
  function formatarCpfFiltro(value) {
  ...
  function getCandidatosDaVaga(vagaId) {
    return candidaturasAdmin.filter((candidato) => String(candidato.vaga_id) === String(vagaId));
  }

  function criarFiltrosVagasAdmin() {
    const lista = document.getElementById("vagas-list");
    if (!lista) return;

    const painel = lista.closest(".panel");
    if (!painel || document.getElementById("admin-vaga-filters")) return;

    const filtros = document.createElement("div");
    filtros.id = "admin-vaga-filters";
    filtros.className = "admin-vaga-filter-panel";
    ...
    filtros.innerHTML = `
      ...
      <p class="admin-vaga-filter-summary" id="admin-vaga-filter-summary">
        Carregando vagas...
      </p>
    `;
    ...
    document.getElementById("admin-vaga-filter-unidade")?.addEventListener("change", renderizarVagasAdmin);
    document.getElementById("admin-vaga-filter-nome")?.addEventListener("input", renderizarVagasAdmin);

    document.getElementById("admin-vaga-filter-cpf")?.addEventListener("input", (event) => {
      event.currentTarget.value = formatarCpfFiltro(event.currentTarget.value);
      renderizarVagasAdmin();
    });
  ...
  function preencherFiltroUnidades() {
    ...
    const unidades = [...new Set(vagasAdmin.map((vaga) => vaga.unidade).filter(Boolean))]
      .sort((a, b) => String(a).localeCompare(String(b), "pt-BR"));

    select.innerHTML = `
      <option value="">Todas as unidades</option>
      ${unidades.map((unidade) => `
        <option value="${escapeAdminHtml(unidade)}">${escapeAdminHtml(unidade)}</option>
      `).join("")}
    `;
  ...
  function filtrarVagasAdmin() {
    const unidadeFiltro = document.getElementById("admin-vaga-filter-unidade")?.value || "";
    const nomeFiltro = normalizarBusca(document.getElementById("admin-vaga-filter-nome")?.value || "");
    const cpfFiltro = somenteNumeros(document.getElementById("admin-vaga-filter-cpf")?.value || "");

    return vagasAdmin.filter((vaga) => {
      if (unidadeFiltro && String(vaga.unidade || "") !== unidadeFiltro) {
        return false;
      }

      const candidatos = getCandidatosDaVaga(vaga.id);

      if (nomeFiltro) {
        const encontrouNome = candidatos.some((candidato) =>
          normalizarBusca(candidato.nome).includes(nomeFiltro)
        );

        if (!encontrouNome) return false;
      }

      if (cpfFiltro) {
        const encontrouCpf = candidatos.some((candidato) =>
          somenteNumeros(candidato.cpf).includes(cpfFiltro)
        );

        if (!encontrouCpf) return false;
      }
  ...
  function renderizarCandidatosDaVaga(vagaId) {
    const candidatos = getCandidatosDaVaga(vagaId);
    ...
    return `
      <details class="admin-candidate-details">
        <summary>Candidatos (${candidatos.length})</summary>

        <div class="admin-candidate-list">
          ${candidatos.map((candidato) => `
            <div class="admin-candidate-chip">
              <strong>${escapeAdminHtml(candidato.nome || "Nome não informado")}</strong>
              <span>CPF: ${escapeAdminHtml(candidato.cpf || "Não informado")}</span>
              ${candidato.telefone ? `<span>Telefone: ${escapeAdminHtml(candidato.telefone)}</span>` : ""}
            </div>
          `).join("")}
        </div>
      </details>
  ...
  function renderizarVagasAdmin() {
    const lista = document.getElementById("vagas-list");
    if (!lista) return;

    criarFiltrosVagasAdmin();
    preencherFiltroUnidades();

    const vagasFiltradas = filtrarVagasAdmin();
    const resumo = document.getElementById("admin-vaga-filter-summary");

    if (resumo) {
      resumo.textContent = `${vagasFiltradas.length} vaga(s) exibida(s) de ${vagasAdmin.length} cadastrada(s).`;
    }

    renderizandoVagasAdmin = true;

    lista.classList.add("admin-vagas-grid");

    if (!vagasFiltradas.length) {
      lista.innerHTML = `<p class="empty-state">Nenhuma vaga encontrada com os filtros aplicados.</p>`;
      renderizandoVagasAdmin = false;
      return;
    }

    lista.innerHTML = vagasFiltradas.map((vaga) => `
      <article class="item-card admin-job-card" data-admin-vaga-id="${escapeAdminHtml(vaga.id)}">
        <div class="item-topline">
          <p class="item-title">${escapeAdminHtml(vaga.cargo || "Cargo não informado")}</p>
          <span class="${vaga.status === "Aberta" ? "tag alert" : "tag"}">
            ${escapeAdminHtml(vaga.status || "Aberta")}
          </span>
        </div>

        <p><strong>Unidade destinada:</strong> ${escapeAdminHtml(vaga.unidade || "Não informada")}</p>

        <div class="job-text-block">
          <strong>Descrição</strong>
          <p>${escapeAdminHtml(vaga.descricao || "Descrição não informada.")}</p>
        </div>

        <div class="job-text-block">
          <strong>Requisitos</strong>
          <p>${escapeAdminHtml(vaga.requisitos || "Requisitos não informados.")}</p>
        </div>

        ${renderizarCandidatosDaVaga(vaga.id)}

        <div class="job-actions">
          <button class="secondary-link" type="button" data-admin-edit-vaga="${escapeAdminHtml(vaga.id)}">
            Editar
          </button>
          <button class="danger-button" type="button" data-admin-delete-vaga="${escapeAdminHtml(vaga.id)}">
            Excluir
          </button>
        </div>
      </article>
  ...
  async function carregarVagasECandidaturasAdmin() {
    ...
    const [vagasResponse, candidaturasResponse] = await Promise.all([
      ...
      supabase
        .from("hub_candidaturas")
        .select("id, vaga_id, nome, cpf, telefone, created_at")
        .order("created_at", { ascending: false }),
    ]);

    if (vagasResponse.error || candidaturasResponse.error) {
      console.error("Erro ao carregar vagas/candidaturas:", vagasResponse.error || candidaturasResponse.error);

      const resumo = document.getElementById("admin-vaga-filter-summary");
      if (resumo) resumo.textContent = "Não foi possível carregar os filtros de vagas.";

      return;
    }

    vagasAdmin = Array.isArray(vagasResponse.data) ? vagasResponse.data : [];
    candidaturasAdmin = Array.isArray(candidaturasResponse.data) ? candidaturasResponse.data : [];

    renderizarVagasAdmin();
  }

  function preencherFormularioEdicaoVaga(id) {
    const vaga = vagasAdmin.find((item) => String(item.id) === String(id));
    ...
    if (submitButton) submitButton.textContent = "Atualizar vaga";

    form.scrollIntoView({ behavior: "smooth", block: "start" });
  }

  async function excluirVagaAdmin(id) {
    const vaga = vagasAdmin.find((item) => String(item.id) === String(id));
    if (!vaga) return;

    const confirmar = window.confirm(
      `Deseja excluir a vaga "${vaga.cargo}" da unidade "${vaga.unidade || "sem unidade"}"?`
    );
    ...
    await carregarVagasECandidaturasAdmin();
  }

  function ativarEventosVagasAdmin() {
    document.addEventListener("click", (event) => {
      const botaoEditar = event.target.closest("[data-admin-edit-vaga]");
      if (botaoEditar) {
        preencherFormularioEdicaoVaga(botaoEditar.dataset.adminEditVaga);
        return;
      }

      const botaoExcluir = event.target.closest("[data-admin-delete-vaga]");
      if (botaoExcluir) {
        excluirVagaAdmin(botaoExcluir.dataset.adminDeleteVaga);
      }
    });

    document.getElementById("vaga-form")?.addEventListener("submit", () => {
      setTimeout(carregarVagasECandidaturasAdmin, 1200);
    });

    document.getElementById("cancelar-edicao-vaga")?.addEventListener("click", () => {
      setTimeout(carregarVagasECandidaturasAdmin, 300);
    });
  }

  function observarRenderizacaoOriginal() {
    const lista = document.getElementById("vagas-list");
    if (!lista || observerAtivo) return;

    const observer = new MutationObserver(() => {
      if (renderizandoVagasAdmin) return;

      window.clearTimeout(window.__adminVagaFilterTimer);
      window.__adminVagaFilterTimer = window.setTimeout(renderizarVagasAdmin, 120);
    });
  ...
  function iniciarMelhoriaVagasAdmin() {
    if (!document.getElementById("vagas")) return;

    criarFiltrosVagasAdmin();
    ativarEventosVagasAdmin();
    observarRenderizacaoOriginal();

    setTimeout(carregarVagasECandidaturasAdmin, 500);
    setTimeout(carregarVagasECandidaturasAdmin, 1800);
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", iniciarMelhoriaVagasAdmin);
  } else {
    iniciarMelhoriaVagasAdmin();
  }
})();

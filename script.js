...
function renderChatAttachmentPreview(files) {
  ...
  if (mimeType.startsWith("image/")) {
    ...
    clearChatPreviewImageSizing(panel);
    const videoPreviewUrl = `${chatAttachmentPreviewUrl}#t=0.1`;
    body = `
      <div class="chat-preview-image-frame chat-preview-video-frame">
        <video class="chat-preview-video" src="${videoPreviewUrl}" controls preload="auto" playsinline aria-label="Previa de ${fileName}"></video>
      </div>`;
    activeChip = `<video class="chat-preview-chip-image" src="${videoPreviewUrl}" muted preload="metadata" playsinline aria-hidden="true"></video>`;
  ...
  const chips = selectedFiles.map((item, index) => {
    ...
    if (index === chatAttachmentPreviewIndex) chipContent = activeChip;
      ...
      chipContent = itemType.startsWith("video/")
        ? `<video class="chat-preview-chip-image" src="${itemUrl}#t=0.1" muted preload="metadata" playsinline aria-hidden="true"></video>`

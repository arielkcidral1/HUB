...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $style '\.chat-panel[\s\S]*grid-template-rows: auto auto minmax\(0, 1fr\) auto[\s\S]*\.chat-panel > \.panel-header[\s\S]*border: 1px solid var\(--line\)[\s\S]*\.chat-feed[\s\S]*border: 1px solid var\(--line\)[\s\S]*\.chat-composer[\s\S]*background: transparent' "chat divide cabecalho mensagens e composer em areas separadas"
  Assert-MatchText $style '\.chat-composer[\s\S]*padding: 6px[\s\S]*\.chat-message-editor[\s\S]*min-height: 34px[\s\S]*\.chat-attach-button[\s\S]*width: 36px[\s\S]*\.send-button[\s\S]*min-height: 36px' "chat composer compacto segue padrao visual do chat"

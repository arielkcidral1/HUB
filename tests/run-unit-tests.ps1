...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $dbApi 'export const PUBLIC_READ_TABLES = new Set\(\["hub_vagas"\]\)' "so vagas ficam legiveis sem sessao em /api/records"
  ...
  Assert-MatchText $authorizeApi 'export function authorizeUsersWrite\(session, \{ method, filters, columns \}\)[\s\S]*if \(isRh\(session\)\) return true[\s\S]*if \(method === "POST"\) return false[\s\S]*const SAFE_SELF_COLUMNS = new Set\(\["nome", "foto_perfil", "configuracoes"\]\)' "so RH cria/edita outros usuarios; conta comum so edita os proprios campos seguros"

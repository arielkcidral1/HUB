...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'async function reauthenticateOnReload\(\)[\s\S]*navigation\?\.type !== "reload"[\s\S]*suspendPresenceOnReload\(previousUser\)[\s\S]*clearStoredSession\(\)[\s\S]*setTimeout\(resolve, 500\)[\s\S]*const authenticated = await reauthenticateInDatabase\(\)[\s\S]*window\.__hubAuthEntryPromise = reauthenticateOnReload\(\)' "F5 reautentica a conta apos 500 ms"

...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'async function reauthenticateOnReload\(\)[\s\S]*navigation\?\.type !== "reload"[\s\S]*suspendPresenceOnReload\(previousUser\)[\s\S]*clearStoredSession\(\)[\s\S]*return reauthenticateInDatabase\(\)[\s\S]*window\.__hubAuthEntryPromise = reauthenticateOnReload\(\)' "F5 reautentica a conta apos a pausa"

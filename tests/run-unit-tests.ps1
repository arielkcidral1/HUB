...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $script 'async function addChatMessage\(values\)[\s\S]*from\(TABLES\.comunicados\)[\s\S]*renderChatChannels\(\)[\s\S]*renderChat\(\)[\s\S]*const results = await Promise\.all\(payloads\.map\(\(payload\) => addChatMessage\(payload\)\)\)' "envio de chat nao faz renderizacao global"

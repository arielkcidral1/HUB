...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $script 'function openRecordContextMenu\(event, type, id\)[\s\S]*const recordsByType = \{ denuncia: data\.denuncias, chamado: data\.chamados, feedback: data\.feedbacks \};[\s\S]*const item = \(recordsByType\[type\] \|\| \[\]\)\.find\(\(record\) => String\(record\.id\) === String\(id\)\);[\s\S]*if \(isArchivedRecord\(item\)\) return;' "menu de contexto nao oferece arquivar para chamado, denuncia ou feedback ja arquivados"
...
Assert-MatchText $script 'function showVtReportMenu\(\).*data-action="gerar-relatorio-vt" data-scope="filtered"[\s\S]*data-action="gerar-relatorio-vt" data-scope="all"' "menu de relatorio VT permite exportar filtrados ou todos"
  ...
  Assert-MatchText $style '\.chamados-filter-bar[\s\S]*max-width: 100%[\s\S]*min-width: 0[\s\S]*flex: 0 1 auto[\s\S]*\.chamados-filter-fields\.vaga-filter-fields[\s\S]*grid-template-columns: 150px 105px 130px[\s\S]*width: auto[\s\S]*\.chamados-filter-fields\.vaga-filter-fields input,[\s\S]*width: 100%' "filtros de vagas cabem dentro do painel"

...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $index 'auth-entry\.js\?v=auth-entry-v11[\s\S]*style\.css\?v=auth-persist-v22[\s\S]*hub-postgres-client\.js\?v=auth-cookie-v1[\s\S]*script\.js\?v=auth-persist-v22[\s\S]*auth-display-guard\.js\?v=auth-display-v2' "HUB autentica sem exibir o painel antes da validacao"
  ...
  Assert-MatchText $script 'const forceReauthentication = new URLSearchParams\(window\.location\.search\)\.get\("reauth"\) === "1"[\s\S]*if \(forceReauthentication && postgresClient\?\.auth\)[\s\S]*await postgresClient\.auth\.signOut\(\)[\s\S]*clearAuthenticatedUser\(\)' "login forcado encerra a sessao anterior e exige novo formulario"

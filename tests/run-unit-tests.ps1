...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $script 'async function addChatMessage\(values\)[\s\S]*from\(TABLES\.comunicados\)[\s\S]*return mapRows\("comunicados", \[inserted\]\)\[0\][\s\S]*const savedMessages = await Promise\.all\(payloads\.map\(\(payload\) => addChatMessage\(payload\)\)\)[\s\S]*const replacements = new Map\(pendingMessages\.map[\s\S]*replacements\.get\(item\.id\) \|\| item[\s\S]*renderChatChannels\(\)[\s\S]*renderChat\(\)' "envio de chat substitui pendente sem sumir"
  Assert-MatchText $script 'async function deleteChatMessageRecord\(id\)[\s\S]*data\.comunicados = current\.filter\(\(item\) => String\(item\.id\) !== String\(id\)\)[\s\S]*renderChatChannels\(\)[\s\S]*renderChat\(\)[\s\S]*from\(TABLES\.comunicados\)[\s\S]*delete\(\)[\s\S]*data\.comunicados = \[removed, \.\.\.\(data\.comunicados \|\| \[\]\)' "exclusao de chat remove localmente sem renderizacao global"

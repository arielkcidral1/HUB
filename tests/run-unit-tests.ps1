...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $style '\.chat-panel[\s\S]*grid-template-rows: auto auto minmax\(0, 1fr\) auto[\s\S]*\.chat-panel > \.panel-header[\s\S]*border: 1px solid var\(--line\)[\s\S]*\.chat-feed[\s\S]*border: 1px solid var\(--line\)[\s\S]*\.chat-composer[\s\S]*box-shadow: var\(--shadow-xs\)' "chat divide cabecalho mensagens e composer em areas separadas"

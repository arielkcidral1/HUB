...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $index 'auth-entry\.js\?v=auth-entry-v6[\s\S]*hub-postgres-client\.js\?v=auth-cookie-v1[\s\S]*script\.js\?v=auth-persist-v20' "HUB valida a entrada antes de carregar o painel"
  ...
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'AUTH_REQUEST_TIMEOUT_MS = 3000[\s\S]*hasStoredIdentity\(\)[\s\S]*fetch\("/api/auth"[\s\S]*credentials: "same-origin"[\s\S]*action: "session"[\s\S]*signal: controller.signal[\s\S]*localIdentityAvailable[\s\S]*window\.__hubAuthEntryPromise' "entrada reautentica sessao no banco e recupera perfil local se houver demora"

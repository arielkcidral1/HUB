...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $script 'function renderFeedbacksSection\(\)[\s\S]*if \(!isFredericoUser\(\)\)[\s\S]*const abertos = \[\.\.\.\(data\.feedbacks \|\| \[\]\)\]\.filter\(\(item\) => !isArchivedRecord\(item\)\);[\s\S]*const arquivados = \[\.\.\.\(data\.feedbacks \|\| \[\]\)\]\.filter\(\(item\) => isArchivedRecord\(item\)\);' "aba de feedbacks separa itens abertos e arquivados"

...
Assert-MatchText $script 'function editarEvento\(id\)[\s\S]*activateView\("calendario"\)[\s\S]*form\.elements\.id\.value = evento\.id[\s\S]*form\.scrollIntoView[\s\S]*focus\(\{ preventScroll: true \}\)' "editar evento pelo contexto abre formulario da aba calendario"
...
Assert-MatchText $script 'function showVtReportMenu\(\).*data-action="gerar-relatorio-vt" data-scope="filtered"[\s\S]*data-action="gerar-relatorio-vt" data-scope="all"' "menu de relatorio VT permite exportar filtrados ou todos"
  ...
  Assert-MatchText $style '\.chamados-filter-fields\.vaga-filter-fields[\s\S]*max-width: 100%[\s\S]*min-width: 0[\s\S]*#vaga-filter-nome[\s\S]*clamp\(118px, 18vw, 150px\)[\s\S]*#vaga-filter-unidade[\s\S]*clamp\(104px, 15vw, 130px\)[\s\S]*\.chamados-filter-fields input,[\s\S]*max-width: 100%[\s\S]*min-width: 0' "filtros de vagas cabem dentro do painel"

...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $index 'auth-entry\.js\?v=auth-entry-v4[\s\S]*hub-postgres-client\.js\?v=auth-cookie-v1[\s\S]*script\.js\?v=auth-persist-v20' "HUB valida a entrada antes de carregar o painel"
  ...
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'RECOVERY_DELAY_MS = 8000[\s\S]*PERSISTED_USER_KEY[\s\S]*POSTGRES_SESSION_KEY[\s\S]*hasRealStoredIdentity\(\)[\s\S]*auth-entry-pending[\s\S]*account-loading\.html\?next=index\.html' "entrada usa sessao local e recupera pagina branca apos falha de carregamento"

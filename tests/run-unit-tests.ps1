...
Assert-MatchText $index 'calendar-event-legend[\s\S]*EV[\s\S]*Evento[\s\S]*AN[\s\S]*Anivers.rio[\s\S]*AE[\s\S]*Anivers.rio de empresa[\s\S]*EN[\s\S]*Entrevista[\s\S]*calendar-event-legend[\s\S]*EV[\s\S]*Evento[\s\S]*AN[\s\S]*Anivers.rio[\s\S]*AE[\s\S]*Anivers.rio de empresa[\s\S]*EN[\s\S]*Entrevista' "painel e calendario exibem indice de tipos de evento"
Assert-MatchText $script 'function getEventIcon\(item = \{\}\)[\s\S]*icon: "AE"[\s\S]*icon: "AN"[\s\S]*icon: "EN"[\s\S]*icon: "EV"[\s\S]*calendar-event-heading[\s\S]*eventIcon\.icon[\s\S]*calendar-event-modal-title' "cards e modal de evento exibem icones distintos por tipo"
...
Assert-MatchText $style '\.calendar-event-legend[\s\S]*\.event-icon-default[\s\S]*#0f766e[\s\S]*\.event-icon-birthday[\s\S]*#92400e[\s\S]*\.event-icon-company[\s\S]*#6d28d9[\s\S]*\.event-icon-interview[\s\S]*#1d4ed8[\s\S]*\.birthday-chip-list button' "icones e indice de eventos possuem cores distintas"

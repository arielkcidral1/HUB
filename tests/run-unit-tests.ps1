...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $loadingScript 'MINIMUM_LOADING_MS = 2000[\s\S]*function finishLoading\([\s\S]*setTimeout\(callback, remaining\)[\s\S]*function redirectToLogin\(\)[\s\S]*POSTGRES_SESSION_KEY[\s\S]*PERSISTED_USER_KEY[\s\S]*removeItem\(key\)[\s\S]*fetch\("/api/auth"[\s\S]*action: "session"[\s\S]*redirectToLogin\(\)' "tela de carregamento aguarda 2 segundos e envia sessao invalida para login"

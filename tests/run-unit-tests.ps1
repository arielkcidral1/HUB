...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'async function reauthenticateOnReload\(\)[\s\S]*navigation\?\.type !== "reload"[\s\S]*clearStoredSession\(\)[\s\S]*return reauthenticateInDatabase\(\)[\s\S]*window\.__hubAuthEntryPromise = reauthenticateOnReload\(\)' "F5 reautentica a conta usando a sessao segura"
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'logout real so deve ocorrer quando o usuario clicar em Desconectar' "F5 nao encerra a sessao persistente antes da reautenticacao"

...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'function disconnectOnReload\(\)[\s\S]*navigation\?\.type !== "reload"[\s\S]*keepalive: true[\s\S]*action: "logout"[\s\S]*window\.__hubReloadSignedOut = true[\s\S]*classList\.remove\("auth-entry-pending"\)[\s\S]*if \(disconnectOnReload\(\)\) return;' "F5 encerra a sessao sem navegar para o login"
  Assert-MatchText $script 'if \(window\.__hubReloadSignedOut\) \{[\s\S]*return false;[\s\S]*clearLegacyTeamCredentials\(\)' "reload desconectado nao inicializa o painel autenticado"

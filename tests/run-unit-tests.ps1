...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $script 'function openRecordContextMenu\(event, type, id\)[\s\S]*const item = \(type === "denuncia" \? data\.denuncias : data\.chamados \|\| \[\]\)\.find\(\(record\) => String\(record\.id\) === String\(id\)\);[\s\S]*if \(isArchivedRecord\(item\)\) return;' "menu de contexto nao oferece arquivar para chamado ou denuncia ja arquivados"

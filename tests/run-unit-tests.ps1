...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'async function suspendPresenceOnReload\(user\)[\s\S]*keepalive: true[\s\S]*online: false' "F5 suspende a presenca no banco"
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'async function reauthenticateOnReload\(\)[\s\S]*navigation\?\.type !== "reload"[\s\S]*suspendPresenceOnReload\(previousUser\)[\s\S]*clearStoredSession\(\)[\s\S]*setTimeout\(resolve, 500\)[\s\S]*return reauthenticateInDatabase\(\)[\s\S]*window\.__hubAuthEntryPromise = reauthenticateOnReload\(\)' "F5 reautentica a conta apos 500 ms"
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'Durante o reload, a presenca fica suspensa e volta ao iniciar o painel' "F5 retorna a presenca apos iniciar o painel"

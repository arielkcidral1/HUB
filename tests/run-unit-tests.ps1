...
Assert-MatchText $style '\.calendar-day\.has-birthday[\s\S]*rgba\(92, 65, 22, 0\.38\)[\s\S]*\.calendar-event-block\.event-type-birthday[\s\S]*border-color: var\(--line\)[\s\S]*border-left-color: var\(--teal-border\)' "aniversarios usam cor apagada sem contorno marrom nos cards"
...
Assert-MatchText $style '\.calendar-event-legend[\s\S]*\.event-icon-default[\s\S]*#0f766e[\s\S]*\.event-icon-birthday[\s\S]*#92400e[\s\S]*\.event-icon-interview[\s\S]*#1d4ed8[\s\S]*\.birthday-chip-list button' "icones e indice de eventos possuem cores distintas"

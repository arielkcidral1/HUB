...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText $index 'auth-entry\.js\?v=auth-entry-v7[\s\S]*hub-postgres-client\.js\?v=auth-cookie-v1[\s\S]*script\.js\?v=auth-persist-v20' "HUB valida a entrada antes de carregar o painel"
  ...
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'AUTH_REQUEST_TIMEOUT_MS = 3000[\s\S]*hasStoredIdentity\(\)[\s\S]*fetch\("/api/auth"[\s\S]*credentials: "same-origin"[\s\S]*action: "session"[\s\S]*signal: controller.signal[\s\S]*if \(localIdentityAvailable\) return true[\s\S]*window\.__hubAuthEntryPromise' "entrada reautentica sessao no banco e recupera perfil local se houver demora"
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'async function hasRecentActivity\(\)[\s\S]*is_online[\s\S]*last_seen[\s\S]*age <= 3000[\s\S]*if \(localIdentityAvailable && await hasRecentActivity\(\)\) return true;' "F5 restaura sessao quando atividade recente esta ativa"
  ...
  Assert-MatchText $script 'window\.__hubAuthEntryPromise[\s\S]*const entryAuthenticated = await window\.__hubAuthEntryPromise[\s\S]*if \(!entryAuthenticated\) return false;' "index aguarda reautenticacao antes de inicializar"

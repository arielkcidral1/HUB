...
function Test-ClientSecurityFunctions {
  ...
  Assert-MatchText (Read-ProjectFile "auth-entry.js") 'AUTH_REQUEST_TIMEOUT_MS = 3000[\s\S]*hasStoredIdentity\(\)[\s\S]*if \(localIdentityAvailable\)[\s\S]*fetch\("/api/auth"[\s\S]*credentials: "same-origin"[\s\S]*action: "session"[\s\S]*if \(result\?\.session\?\.user\) persistAuthenticatedSession\(result\.session\)[\s\S]*return true[\s\S]*window\.__hubAuthEntryPromise' "F5 restaura a sessao local antes da validacao remota"
